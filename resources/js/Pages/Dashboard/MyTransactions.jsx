    ...
    return (
        <UserLayout>
            ...
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black text-slate-800 leading-tight font-display">My Transactions</h2>
                <Link href={route('dashboard')} className="text-brand-pink font-bold text-xs uppercase tracking-widest hover:underline">
                    Back to Dashboard
                </Link>
            </div>

            <div className="bg-white rounded-[40px] shadow-xl shadow-pink-100/50 overflow-hidden border border-pink-50">
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-600">
                        <thead className="bg-pink-50/50 text-brand-pink font-display uppercase text-xs font-black tracking-wider">
                            <tr>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wide
                                                ${trx.status === 'success' ? 'bg-green-100 text-green-600' :
                                                    trx.status === 'failed' ? 'bg-red-100 text-red-600' :
                                                        'bg-yellow-100 text-yellow-600'}`}>
                                                {trx.status === 'success' && <CheckCircle className="w-3 h-3 mr-1" />}
                                                {trx.status === 'failed' && <AlertCircle className="w-3 h-3 mr-1" />}
                                                {trx.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
